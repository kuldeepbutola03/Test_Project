...
export default class HomeScreen extends Component {
  ...
  tick3 = () => {
    if (!this.state.isLoading && this._carousel) {

      let sliderP = ((this.state.activeSlide + 1) >= this.state.slideContent.length) ? 0 : (this.state.activeSlide + 1);
  ...
  serverHitForFirstApi() {
    ...
    fetch(LANDING_RESOURCES, {
      ...
      .then((responseJson) => {
        ...
        // else{
        //   arr[4] = require('../../assets/PollResults/poll_result.jpg');
        // }
        // activeSlide : 0

        let objetc = Object.assign([], responseJson.resCatNationalList ? responseJson.resCatNationalList : []);
        if (responseJson.userMaster) {
          // alert(responseJson.userMaster.userLanguage);
          let dict = {};
          dict["image"] = responseJson.userMaster.userImageData ? responseJson.userMaster.userImageData : this.state.data.image;
          dict["username"] = responseJson.userMaster.userName;
          dict["firstName"] = responseJson.userMaster.userFirstName;
          dict["lastName"] = responseJson.userMaster.userLastName;
          dict["selectedAgeGroupCode"] = responseJson.userMaster.userAgeGroup;
          dict["gender"] = responseJson.userMaster.userGender;
          dict["userId"] = responseJson.userMaster.userId;
          dict["description"] = responseJson.userMaster.userDescription;
          dict["userDesignation"] = responseJson.userMaster.userDesignation;
          dict["userLanguage"] = responseJson.userMaster.userLanguage;

          // this.setState({ data: dict });

          saveUserData(dict);
          this.setState({ isLoading: false, firstAPIresponse: responseJson, currLandPageSurvey: responseJson.currLandPageSurvey, slideContent: arr, data: dict, topSixContent: objetc.splice(0, 6) });
        } else {
          this.getUserDetails();
          this.setState({ isLoading: false, firstAPIresponse: responseJson, currLandPageSurvey: responseJson.currLandPageSurvey, slideContent: arr, topSixContent: objetc.splice(0, 6) }); // slideContent : arr
        }
  ...
  _renderItem({ item, index }) {
    const { width, height, firstAPIresponse } = this.state;
      ...
      >
        ...
        <View
          ...
          {!this.state.isLoading && (!check ?
            ...
            <TextTicker style={styles.landingTopSixHeader}
              // duration={this.getDuration(this.state.firstAPIresponse.nationalTicker ? this.state.firstAPIresponse.nationalTicker.length : 0)}
              loop
              bounce={false}
              repeatSpacer={50}
              // marqueeDelay={1000}
          ...
          <View style={styles.buttonContainer}>
            ...
            {!this.state.isLoading && (!check ?
              ...
              <TextTicker style={styles.landingTopSixHeader}
                // duration={this.getDuration(this.state.firstAPIresponse.stateTicker ? this.state.firstAPIresponse.stateTicker.length : 0)}
                loop
                bounce={false}
                repeatSpacer={50}
                easing={Easing.linear}
